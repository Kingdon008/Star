...
public class MTPopMenu: UIView {
    ...
    func calculateWidth() {
        for model in self.menuArray {
            let rect = String.ss.getTextRectSize(text: model.title!,font: self.titleFont,size: CGSize.init(width: kScreenWidth, height: 20))
...
public extension MTPopMenu {
    ...
    func popMenu(anchorView: UIView, menuArray: [MTPopMenuModel]) {
