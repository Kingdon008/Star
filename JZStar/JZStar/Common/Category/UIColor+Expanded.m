...
UIColor *RandomColor() {
  ...
  return [UIColor colorWithRed:arc4random() / (CGFloat)UINT32_MAX
                         green:arc4random() / (CGFloat)UINT32_MAX
                          blue:arc4random() / (CGFloat)UINT32_MAX
                         alpha:1.0f];
}

UIColor *InterpolateColors(UIColor *c1, UIColor *c2, CGFloat amt) {
  CGFloat r = (CGFloat)((c2.red * amt) + (c1.red * (1.0 - amt)));
  CGFloat g = (CGFloat)((c2.green * amt) + (c1.green * (1.0 - amt)));
  CGFloat b = (CGFloat)((c2.blue * amt) + (c1.blue * (1.0 - amt)));
  CGFloat a = (CGFloat)((c2.alpha * amt) + (c1.alpha * (1.0 - amt)));
...
+ (UIImage *)colorWheelOfSize:(CGFloat)side border:(BOOL)useBorder {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    for (int j = 0; j < 12; ++j) {
      CGFloat theta = (CGFloat)(M_PI / 6 * j);
      CGFloat hue = (CGFloat)(theta / (2 * M_PI));
      UIColor *c = [UIColor colorWithHue:hue
...
- (void)toC:(CGFloat *)cyan
  ...
  CGFloat k = 1.0f - (CGFloat)fmax(fmax(r, g), b);
  CGFloat dK = 1.0f - k;
...
- (CGFloat)cyanChannel {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -cyanChannel");
  CGFloat c = 0.0f;
...
- (CGFloat)magentaChannel {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -magentaChannel");
  CGFloat m = 0.0f;
...
- (CGFloat)yellowChannel {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -yellowChannel");
  CGFloat y = 0.0f;
...
- (CGFloat)blackChannel {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -blackChannel");
  CGFloat k = 0.0f;
...
+ (void)hue:(CGFloat)h
  ...
  if (s == 0.0f) {
    ...
    int i = (int)floor(h); // largest integer <= h
...
- (CGFloat)white {
  NSAssert(self.usesMonochromeColorspace,
           @"Must be a Monochrome color to use -white");
...
- (CGFloat)saturation {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -saturation");
  CGFloat s = 0.0f;
...
- (CGFloat)brightness {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -brightness");
  CGFloat v = 0.0f;
...
- (CGFloat)luminance {
  NSAssert(self.canProvideRGBComponents,
           @"Must be a RGB color to use -luminance");
...
- (CGFloat)premultipliedRed {
  return self.red * self.alpha;
}
- (CGFloat)premultipliedGreen {
  return self.green * self.alpha;
}
- (CGFloat)premultipliedBlue {
  return self.blue * self.alpha;
}

- (Byte)redByte {
  return MAKEBYTE(self.red);
}
- (Byte)greenByte {
  return MAKEBYTE(self.green);
}
- (Byte)blueByte {
  return MAKEBYTE(self.blue);
}
- (Byte)alphaByte {
  return MAKEBYTE(self.alpha);
}
- (Byte)whiteByte {
  return MAKEBYTE(self.white);
};
...
- (NSData *)premultipledColorBytes {
  Byte bytes[4] = {MAKEBYTE(self.alpha), MAKEBYTE(self.premultipliedRed),
                   MAKEBYTE(self.premultipliedGreen),
                   MAKEBYTE(self.premultipliedBlue)};
  NSData *data = [NSData dataWithBytes:bytes length:4];
  return data;
}
...
- (NSArray *)arrayFromRGBAComponents {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -arrayFromRGBAComponents");
...
- (CGFloat)colorfulness {
  CGFloat d1 = (CGFloat)fabs(self.red - self.green);
  CGFloat d2 = (CGFloat)fabs(self.green - self.blue);
  CGFloat d3 = (CGFloat)fabs(self.blue - self.red);
...
- (CGFloat)warmth {
  ...
  CGFloat distance = (CGFloat)fabs(hue);
...
- (UIColor *)colorByMultiplyingByRed:(CGFloat)red
                               ...
                               alpha:(CGFloat)alpha {
  NSAssert(self.canProvideRGBComponents,
           @"Must be a RGB color to use arithmetic operations");
...
- (UIColor *)colorByInterpolatingToColor:(UIColor *)color
  ...
  NSAssert(self.canProvideRGBComponents,
           @"Self must be a RGB color to use arithmatic operations");
  NSAssert(color.canProvideRGBComponents,
           @"Color must be a RGB color to use arithmatic operations");
...
// Pick two colors more colors such that all three are equidistant on the color
// wheel
...
// Pick n pairs of colors, stepping in increasing steps away from this color
// around the wheel
- (NSArray *)analogousColorsWithStepAngle:(CGFloat)stepAngle
                                pairCount:(NSInteger)pairs {
  // Convert to HSB
  CGFloat h = self.hue * 360.0f;
  CGFloat s = self.saturation;
  CGFloat v = self.brightness;
  ...
  NSMutableArray *colors =
      [NSMutableArray arrayWithCapacity:(NSUInteger)pairs * 2];
  ...
  for (int i = 1; i <= pairs; ++i) {
    ...
    [colors addObject:[UIColor colorWithHue:h1 / 360.0f
                                 saturation:s
                                 brightness:v
                                      alpha:a]];
    [colors addObject:[UIColor colorWithHue:h2 / 360.0f
                                 saturation:s
                                 brightness:v
                                      alpha:a]];
  }
...
//  - Eridius - UIColor needs a method that takes 2 colors and gives a third
//  complementary one
- (UIColor *)kevinColorWithColor:(UIColor *)secondColor {
  CGFloat startingHue = cgfmin(self.hue, secondColor.hue);
  CGFloat distance = (CGFloat)fabs(self.hue - secondColor.hue);
  if (distance > 0.5) {
    distance = 1 - distance;
  ...
  return
      [UIColor colorWithHue:target saturation:sat brightness:bri alpha:alpha];
}
...
void RGBtoHSP(CGFloat R, CGFloat G, CGFloat B, CGFloat *H, CGFloat *S,
  ...
  if ((H == NULL) || (S == NULL) || (P == NULL)) {
    ...
    fprintf(
        stderr,
        "Sorry. Please call RGBtoHSP with non-NULL H, S, and P.  Bailing.\n");
    return;
  }
  ...
  *P = (CGFloat)sqrt(R * R * Pr + G * G * Pg + B * B * Pb);
  ...
  if (R >= G && R >= B) { //  R is largest
    if (B >= G) {
      *H = (CGFloat)(6. / 6. - 1. / 6. * (B - G) / (R - G));
      *S = (CGFloat)(1. - (G / R));
    } else {
      *H = (CGFloat)(0. / 6. + 1. / 6. * (G - B) / (R - B));
      *S = (CGFloat)(1. - B / R);
    }
  } else if (G >= R && G >= B) { //  G is largest
    if (R >= B) {
      *H = (CGFloat)(2. / 6. - 1. / 6. * (R - B) / (G - B));
      *S = (CGFloat)(1. - B / G);
    } else {
      *H = (CGFloat)(2. / 6. + 1. / 6. * (B - R) / (G - R));
      *S = (CGFloat)(1. - R / G);
    }
  } else { //  B is largest
    if (G >= R) {
      *H = (CGFloat)(4. / 6. - 1. / 6. * (G - R) / (B - R));
      *S = (CGFloat)(1. - R / B);
    } else {
      *H = (CGFloat)(4. / 6. + 1. / 6. * (R - G) / (B - G));
      *S = (CGFloat)(1. - G / B);
    }
  }
}
...
void HSPtoRGB(CGFloat H, CGFloat S, CGFloat P, CGFloat *R, CGFloat *G,
  ...
  if ((R == NULL) || (G == NULL) || (B == NULL)) {
    // It is too much of a pain to check the referencing for each of these bits.
    fprintf(stderr, "Sorry. Please call with HSPtoRGB with non-NULL R, G, and "
                    "B.  Bailing.\n");
    return;
  }

  CGFloat part, minOverMax = 1.0f - (CGFloat)S;
  ...
  if (minOverMax > 0.) {
    if (H < 1. / 6.) { //  R>G>B
      ...
      *B = P /
           (CGFloat)sqrt(Pr / minOverMax / minOverMax + Pg * part * part + Pb);
      *R = (*B) / minOverMax;
      *G = (*B) + H * ((*R) - (*B));
    } else if (H < 2. / 6.) { //  G>R>B
      H = 6.f * (-H + 2.f / 6.f);
      part = 1.f + H * (1.f / minOverMax - 1.f);
      *B = P /
           (CGFloat)sqrt(Pg / minOverMax / minOverMax + Pr * part * part + Pb);
      *G = (*B) / minOverMax;
      *R = (*B) + H * ((*G) - (*B));
    } else if (H < 3. / 6.) { //  G>B>R
      H = 6.f * (H - 2.f / 6.f);
      part = 1.f + H * (1.f / minOverMax - 1.f);
      *R = P /
           (CGFloat)sqrt(Pg / minOverMax / minOverMax + Pb * part * part + Pr);
      *G = (*R) / minOverMax;
      *B = (*R) + H * ((*G) - (*R));
    } else if (H < 4. / 6.) { //  B>G>R
      H = 6.f * (-H + 4.f / 6.f);
      part = 1.f + H * (1.f / minOverMax - 1.f);
      *R = P /
           (CGFloat)sqrt(Pb / minOverMax / minOverMax + Pg * part * part + Pr);
      *B = (*R) / minOverMax;
      *G = (*R) + H * ((*B) - (*R));
      ...
      part = 1.f + H * (1.f / minOverMax - 1.f);
      *G = P /
           (CGFloat)sqrt(Pb / minOverMax / minOverMax + Pr * part * part + Pg);
      *B = (*G) / minOverMax;
      *R = (*G) + H * ((*B) - (*G));
    } else { //  R>B>G
      H = 6.f * (-H + 6.f / 6.f);
      part = 1.f + H * (1.f / minOverMax - 1.f);
      *G = P /
           (CGFloat)sqrt(Pr / minOverMax / minOverMax + Pb * part * part + Pg);
      *R = (*G) / minOverMax;
      *B = (*G) + H * ((*R) - (*G));
    }
  } else {
...
- (uint32_t)rgbHex {
  ...
  // getRed:green:blue:alpha: now has contractual 0.0 to 1.0 according to docs,
  // so removed clamping

  // Thanks gwynne
  // return (((int)roundf(r * 0xFF)) << 16) | (((int)roundf(g * 0xFF)) << 8) |
  // (((int)roundf(b * 0xFF)));
  return (uint32_t)((lrint(r * 0xFF) << 16) | (lrint(g * 0xFF) << 8) |
                    (lrint(b * 0xFF)));
}

- (NSString *)stringValue {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -stringValue");
  NSString *result;
  switch (self.colorSpaceModel) {
  case kCGColorSpaceModelRGB:
    result =
        [NSString stringWithFormat:@"{%0.4f, %0.4f, %0.4f, %0.4f}", self.red,
                                   self.green, self.blue, self.alpha];
    break;
  case kCGColorSpaceModelMonochrome:
    result =
        [NSString stringWithFormat:@"{%0.4f, %0.4f}", self.white, self.alpha];
    break;
  default:
    result = nil;
  }
...
- (NSString *)hexStringValue {
  NSAssert(self.canProvideRGBComponents,
           @"Must be an RGB color to use -hexStringValue");
  NSString *result;
  switch (self.colorSpaceModel) {
  case kCGColorSpaceModelRGB:
    result = [NSString stringWithFormat:@"%02X%02X%02X", self.redByte,
                                        self.greenByte, self.blueByte];
    break;
  case kCGColorSpaceModelMonochrome:
    result = [NSString stringWithFormat:@"%02X%02X%02X", self.whiteByte,
                                        self.whiteByte, self.whiteByte];
    break;
  default:
    result = nil;
  }
...
- (NSString *)valueString {
  return [NSString
      stringWithFormat:@"%@ [%d %d %d] (%@): RGB:(%f, %f, %f) HSB:(%f, %f, %f) "
                       @"CMYK:(%@) alpha: %f",
                       self.hexStringValue, self.redByte, self.greenByte,
                       self.blueByte, self.closestColorName, self.red,
                       self.green, self.blue, self.hue, self.saturation,
                       self.brightness,
                       [self.cmyk componentsJoinedByString:@", "], self.alpha];
}
...
+ (UIColor *)colorWithKelvin:(CGFloat)kelvin {
  ...
  if (temperature <= 66) {
    red = 0xFF;
    green = temperature;
    green = 99.4708025861f * (CGFloat)log(green) - 161.1195681661f;
  } else {
    red = temperature - 60;
    red = 329.698727446f * (CGFloat)pow(red, -0.1332047592f);
    green = temperature - 60;
    green = 288.1221695283f * (CGFloat)pow(green, -0.0755148492f);
  }
  ...
  if (temperature >= 66)
    ...
    blue = temperature - 10;
    blue = 138.5177312231f * (CGFloat)log(blue) - 305.0447927307f;
  }
...
/*
 Photographers and lighting designers speak of color temperatures in "degrees
 kelvin." For example, 3200K represents a typical indoor color temperature and
 5500K represents typical daylight color temperature. In the context of
 lighting, a specific kelvin temperature expresses the color temperature (dull
 red, bright red, white, blue) corresponding to the physical temperature (warm,
 hot, extremely hot) of an object.
...
- (CGFloat)colorTemperature {
  ...
  for (NSString *hexKey in kelvinDictionary.allKeys) {
    UIColor *color = [UIColor colorWithHex:hexKey];
...
// Returns a UIColor by scanning the string for a hex number and passing that to
...
+ (UIColor *)randomDarkColor:(CGFloat)scaleFactor {
  ...
  return [UIColor colorWithRed:scaleFactor * arc4random() / (CGFloat)UINT32_MAX
                         green:scaleFactor * arc4random() / (CGFloat)UINT32_MAX
                          blue:scaleFactor * arc4random() / (CGFloat)UINT32_MAX
                         alpha:1.0f];
}
...
+ (UIColor *)randomLightColor:(CGFloat)scaleFactor {
  ...
  return [UIColor
      colorWithRed:difference + scaleFactor * arc4random() / (CGFloat)UINT32_MAX
             green:difference + scaleFactor * arc4random() / (CGFloat)UINT32_MAX
              blue:difference + scaleFactor * arc4random() / (CGFloat)UINT32_MAX
             alpha:1.0f];
}
@end
...
+ (void)initializeColorDictionaries {
  ...
  NSDictionary *systemColorDictionary = @{
    @"Black" : @"000000",
    @"Dark Gray" : @"555555",
    @"Light Gray" : @"AAAAAA",
    @"White" : @"FFFFFF",
  ...
  NSDictionary *wikipediaColorDictionary = @{
    @"Aero" : @"7CB9E8",
    @"Aero Blue" : @"C9FFE5",
   ...
   */
  NSDictionary *moroneyDictionary = @{
    @"Apple Green" : @"58e24a",
    @"Apricot" : @"ffa863",
    @"Aqua" : @"42dad3",
    @"Aqua Blue" : @"4acfee",
  ...
  /*
   ...
   Known items *not* addressed are: orangeish, camo, azul, burple, purpleish,
   camo green, and blurple.
   */
  NSDictionary *xkcdDictionary = @{
    @"Acid Green" : @"8ffe09",
    @"Adobe" : @"bd6c48",
    @"Algae" : @"54ac68",
    @"Algae Green" : @"21c36f",
    ...
    @"Baby Poo" : @"ab9004",
    @"Baby Poop" : @"937c00",
    @"Baby Poop Brown" : @"ad900d",
    @"Baby Poop Green" : @"889717",
    ...
    @"Dark Salmon" : @"c85a53",
    @"Dark Sand" : @"a88f59",
    @"Dark Sea Foam" : @"1fb57a",
    @"Dark Sea Foam Green" : @"3eaf76",
    ...
    @"Light Gray" : @"d8dcd6",
    @"Light Gray Blue" : @"9dbcd4",
    @"Light Gray Green" : @"b7e1a1",
    @"Light Green" : @"76ff7b",
    ...
    @"Sea Blue" : @"047495",
    @"Sea Foam" : @"80f9ad",
    @"Sea Foam Blue" : @"78d1b6",
    @"Sea Foam Green" : @"7af9ab",
    @"Sea Green" : @"53fca1",
    @"Seaweed" : @"18d17b",
...
+ (UIColor *)colorWithName:(NSString *)name
  ...
  return [UIColor colorWithHex:hexkey];
}
...
- (NSString *)closestColorNameUsingDictionary:(NSString *)dictionaryName {
  NSAssert(self.canProvideRGBComponents,
           @"Must be a RGB color to use closestColorName");
  if (!dictionaryName) {
  ...
  for (NSString *colorName in colorDictionary.allKeys) {
    ...
    UIColor *comparisonColor = [UIColor colorWithHex:colorHex];
    if (!comparisonColor)
      continue;
...
- (NSString *)closestColorName {
  ...
  for (NSString *dictionaryName in [UIColor availableColorDictionaries]) {
    ...
    if ([dictionaryName isEqualToString:@"PseudoPantone"])
      continue;

    NSString *colorString =
        [self closestColorNameUsingDictionary:dictionaryName];
    if (!colorString)
      continue;

    UIColor *color =
        [UIColor colorWithName:colorString inDictionary:dictionaryName];
    CGFloat distance = [self distanceFrom:color];
...
- (UIColor *)closestMensColor {
  // Even more limited
  // NSArray *baseColors = @[[UIColor redColor], [UIColor greenColor], [UIColor
  // blueColor], [UIColor yellowColor], [UIColor orangeColor], [UIColor
  // purpleColor]];
  ...
  NSArray *grayColors = @[
    [UIColor blackColor], [UIColor lightGrayColor], [UIColor grayColor],
  ...
  for (UIColor *color in colors) {
    ...
    CGFloat score = evaluateAsGray ? [self distanceFrom:color]
                                   : [self hueDistanceFrom:color];
...
+ (void)initializeColorNames {
  ...
  for (NSString *dictionaryName in [UIColor availableColorDictionaries]) {
    if ([dictionaryName hasPrefix:@"PseudoPantone"])
      continue; // No Pantone please
  ...
  // Sort and unique
  NSArray *sorted =
      [baseNames sortedArrayUsingComparator:^(NSString *obj1, NSString *obj2) {
...
+ (NSArray *)closeColorNamesMatchingKeys:(NSArray *)keys {
  ...
  for (NSString *key in keys) {
    NSPredicate *containPred =
        [NSPredicate predicateWithFormat:@"SELF contains[cd] %@", key];
...
+ (NSArray *)closeAppleNamesMatchingKeys:(NSArray *)keys {
  ...
  NSArray *results = colorNames;
  for (NSString *key in keys) {
    NSPredicate *containPred = [NSPredicate
        predicateWithFormat:
            @"SELF contains[cd] %@ AND SELF BEGINSWITH[c] 'Apple'", key];
    NSArray *filtered = [results filteredArrayUsingPredicate:containPred];
    results = filtered;
  }
  return results;
